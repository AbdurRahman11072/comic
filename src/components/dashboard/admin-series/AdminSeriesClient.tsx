...
export function AdminSeriesClient({
  ...
  const [seriesList, setSeriesList] = useState<AdminSeriesItem[]>(initialSeries);
  ...
  // Delete Modal State
  const [seriesToDelete, setSeriesToDelete] = useState<AdminSeriesItem | null>(null);
  ...
  useEffect(() => {
    // Only re-fetch on parameter updates after initial render
    if (page !== 1 || statusFilter !== "ALL" || typeFilter !== "ALL" || hiddenFilter !== "all" || sort !== "latest") {
      fetchSeries();
    }
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <Link
                          ...
                          className="p-2 rounded-lg glass glass-hover text-cyan-400/80 hover:text-cyan-400 hover:bg-cyan-500/10"
          ...
          <div className="bg-neutral-900 border border-red-500/20 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 rounded-xl text-xs font-bold text-white/70 hover:text-white glass cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-red-500 hover:bg-red-600 transition flex items-center gap-1.5 shadow-lg shadow-red-500/20 cursor-pointer"
              >
