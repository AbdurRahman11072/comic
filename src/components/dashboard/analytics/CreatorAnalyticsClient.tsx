...
export function CreatorAnalyticsClient({ initialData = null }: CreatorAnalyticsClientProps) {
    ...
    <div className="space-y-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  ) : (selectedSeries as any).attentionStatus === "TRENDING" ? (
                ...
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-2">
                  <span>Updated {(selectedSeries as any).daysSinceUpdate ?? 1} days ago</span>
                  <span>•</span>
                  <span>{(selectedSeries as any)._count?.chapters ?? (selectedSeries as any).chaptersCount ?? 0} Chapters published</span>
                </p>
              </div>
            </div>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    {(selectedSeries as any).attentionReason || "Performance is consistent with expected readership trends."}
                  </p>
                </div>
              </div>
            </div>
        ...
        {[
          {
            ...
            value: selectedSeries ? ((selectedSeries as any)._count?.chapters ?? (selectedSeries as any).chaptersCount ?? 0) : overview.totalSeries,
            icon: BookOpen,
            color: "text-blue-400",
            bg: "bg-blue-400/10"
          },
          {
            label: "Total Views",
            value: selectedSeries ? ((selectedSeries as any).totalViews ?? (selectedSeries as any).views ?? 0).toLocaleString() : overview.totalViews.toLocaleString(),
            icon: Eye,
          ...
          {
            label: "Bookmarks",
            value: selectedSeries ? ((selectedSeries as any)._count?.bookmarks ?? (selectedSeries as any).likesCount ?? 0) : ((overview as any).totalBookmarks ?? overview.totalLikes),
            icon: Heart,
            color: "text-pink-400",
            bg: "bg-pink-400/10"
          },
          {
            ...
            value: selectedSeries ? `${(selectedSeries as any).bookmarkRate ?? 0}%` : ((overview as any).totalReviews ?? overview.totalComments),
          ...
          {
            label: "Rating",
            value: selectedSeries ? `${selectedSeries.rating.toFixed(1)} ★` : `${overview.averageRating ? overview.averageRating.toFixed(1) : "4.8"} ★`,
            icon: Star,
          ...
          {
            label: "Earnings",
            value: `${(selectedSeries ? selectedSeries.earnings : (overview.totalEarnings ?? (overview as any).totalRevenue ?? 0)).toLocaleString()} P`,
            icon: DollarSign,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass rounded-3xl p-6 border border-amber-500/20 bg-gradient-to-b from-amber-500/[0.03] to-transparent">
          ...
          <div className="space-y-3">
            {series.slice(0, 3).map((s: any, idx) => (
              <div
                key={s.id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-muted-foreground mt-0.5">
                      <span className="text-amber-400 font-mono font-semibold">{(s.totalViews ?? s.views ?? 0).toLocaleString()} views</span>
                      <span>•</span>
                      <span className="text-pink-400 font-mono font-semibold">{s._count?.bookmarks ?? s.likesCount ?? 0} saves</span>
                    </div>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-extrabold text-emerald-400 font-mono">{(s.earnings ?? 0).toLocaleString()} P</p>
            ...
            <tbody>
              {series.map((s: any) => (
                ...
                >
                  <td className="py-4 pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-muted-foreground uppercase">{s.type || "MANHWA"}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-4 text-center font-mono font-semibold">{s._count?.chapters ?? s.chaptersCount ?? 0}</td>
