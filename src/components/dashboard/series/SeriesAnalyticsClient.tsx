...
import { SingleSeriesAnalyticsData } from "@/services/creator.service";
...
export function SeriesAnalyticsClient({
  ...
  if (!data) {
    return (
      <div className="text-center py-24 space-y-5">
        ...
        <h2 className="text-xl font-bold text-white">Series Analytics Unavailable</h2>
        ...
        <Link href="/dashboard/series">
          <button className="px-5 py-2.5 bg-white/10 hover:bg-white/15 text-white font-bold rounded-xl text-sm transition cursor-pointer">
  ...
  const { series, metrics, chapters, revenueChart, recentReviews } = data as any;

  // Chart data for chapters performance comparison
  const chapterChartData = (chapters || []).map((c: any) => ({
    name: `Ch. ${c.number}`,
    unlocks: c.unlocksCount || 0,
    earnings: c.earnings || 0,
    readers: c.readersCount || 0,
  }));

  const attentionStatus = metrics?.attentionStatus || "STEADY";
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-start sm:items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1.5 flex items-center gap-2 flex-wrap">
                <span>Updated {metrics?.daysSinceUpdate ?? 1} days ago</span>
                <span>•</span>
                <span>Created {new Date(series.createdAt).toLocaleDateString()}</span>
                {Array.isArray(series.genres) && series.genres.length > 0 && (
                  <>
                    <span>•</span>
                    <span className="text-primary/90">{series.genres.map((g: any) => typeof g === 'string' ? g : g.name).join(", ")}</span>
                  </>
                )}
              </p>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: "Total Views", value: (metrics?.totalViews ?? series.totalViews ?? 0).toLocaleString(), icon: Eye, color: "text-amber-400", bg: "bg-amber-400/10" },
          { label: "Chapters", value: metrics?.totalChapters ?? (chapters?.length || 0), icon: BookOpen, color: "text-blue-400", bg: "bg-blue-400/10" },
          { label: "Bookmarks", value: (metrics?.totalBookmarks ?? series.bookmarksCount ?? 0).toLocaleString(), icon: Heart, color: "text-pink-400", bg: "bg-pink-400/10" },
          { label: "Save Rate", value: `${metrics?.bookmarkRate ?? 0}%`, icon: TrendingUp, color: "text-cyan-400", bg: "bg-cyan-400/10" },
          { label: "Reader Rating", value: `${(series.rating || 5.0).toFixed(1)} ★`, icon: Star, color: "text-yellow-400", bg: "bg-yellow-400/10" },
          { label: "Total Earnings", value: `${(metrics?.totalEarnings ?? 0).toLocaleString()} P`, icon: DollarSign, color: "text-emerald-400", bg: "bg-emerald-400/10" },
        ].map((stat) => {
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chapterChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="unlocks" fill="#06b6d4" radius={[6, 6, 0, 0]}>
                    {chapterChartData.map((entry: any, index: number) => (
      ...
      <div className="glass rounded-3xl p-6 border border-white/5 shadow-xl">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href={`/dashboard/chapters/add?seriesId=${series.id}`}>
            <button className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold border border-white/10 flex items-center gap-1.5 transition cursor-pointer">
      ...
      {recentReviews && recentReviews.length > 0 && (
        <div className="glass rounded-3xl p-6 border border-white/5 shadow-xl">
          <h2 className="text-lg font-bold text-white mb-4">Recent Reader Reviews ({series.reviewsCount || recentReviews.length})</h2>
              ...
              <div key={rev.id} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">{rev.user?.name || "Reader"}</span>
                  </div>
