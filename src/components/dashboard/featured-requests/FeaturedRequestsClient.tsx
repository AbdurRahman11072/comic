...
import { Sparkles, Check, X, Loader2, AlertCircle, Plus, Coins, Calendar } from "lucide-react";
import { userService } from "@/services/user.service";
import { creatorService, FeaturedRequestItem } from "@/services/creator.service";
import { seriesService } from "@/services/series.service";
import { RequestFeatureSeriesAction, ReviewFeaturedRequestAction } from "@/actions/creator";
...
interface FeaturedRequestsClientProps {
  initialRequests?: FeaturedRequestItem[];
  initialSeriesList?: SeriesOption[];
  initialUserPoints?: number;
  initialFee?: number;
  initialRole?: string;
}

export function FeaturedRequestsClient({
  initialRequests = [],
  initialSeriesList = [],
  initialUserPoints = 0,
  initialFee = 500,
  initialRole = "user",
}: FeaturedRequestsClientProps) {
  const { data: session } = authClient.useSession();
  const [requests, setRequests] = useState<FeaturedRequestItem[]>(initialRequests);
  const [loading, setLoading] = useState(false);
  const [actionLoading, setActionLoading] = useState<string | null>(null);

  // Creator-specific states
  const [showModal, setShowModal] = useState(false);
  const [seriesList, setSeriesList] = useState<SeriesOption[]>(initialSeriesList);
  const [selectedSeriesId, setSelectedSeriesId] = useState("");
  const [durationDays, setDurationDays] = useState<number>(7);
  const [notes, setNotes] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [fee, _setFee] = useState(initialFee);
  ...
  const fetchData = async (targetStatus = filterStatus) => {
    setLoading(true);
    try {
      if (isCreator) {
        ...
        // Fetch creator's series if empty
        if (session?.user?.id && seriesList.length === 0) {
          const seriesRes = await seriesService.getAllSeries({ creatorId: session.user.id, limit: 100 });
        ...
        // Fetch all requests
        const res = await creatorService.getModeratorFeaturedRequests({ status: targetStatus });
  ...
  const handleReview = async (id: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      const res = await ReviewFeaturedRequestAction(id, {
        status,
        notes: notesText,
      });
      ...
      if (res.success) {
        toast.success(
          status === "APPROVED"
            ? "Request approved & series featured!"
            : "Request rejected & points refunded to creator."
        );
        setReviewNotes((prev) => {
          const copy = { ...prev };
          delete copy[id];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {isCreator && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:opacity-90 transition-all active:scale-95 cursor-pointer"
          >
                ...
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex-1 md:flex-none px-4 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/30 rounded-lg transition text-xs font-bold flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={actionLoading === req.id}
                      className="flex-1 md:flex-none px-4 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-lg transition text-xs font-bold flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50"
                    >
          ...
          <div className="relative glass border border-white/10 rounded-3xl w-full max-w-lg p-6 overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                disabled={submitting}
                className="p-1 hover:bg-white/10 rounded-full transition text-muted-foreground hover:text-white cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmitRequest} className="space-y-5">
              ...
              <div className="flex gap-3 justify-end pt-2">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl text-xs font-bold shadow-lg shadow-primary/20 hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
