...
const recordSeriesView = asyncHandler(async (req: Request, res: Response) => {
  const { slug } = req.params;
  const clientIp =
  ...
  const result = await SeriesService.recordSeriesView(slug as string, clientIp, userId, userAgent);

  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
    message: result.counted ? 'Series view recorded' : 'View already counted or skipped',
    data: result,
  });
});
