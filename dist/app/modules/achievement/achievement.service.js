...
const getAchievements = async (userId) => {
    ...
    const userUnlocked = await prisma_1.prisma.userAchievement.findMany({
        where: { userId },
...
const checkAndUnlockAchievements = async (userId) => {
    ...
    const unlocked = await prisma_1.prisma.userAchievement.findMany({
        where: { userId },
    ...
    const [adCount, buyCount, user] = await Promise.all([
        prisma_1.prisma.pointTransaction.count({
            where: { userId, type: 'EARN_AD' }
        }),
        prisma_1.prisma.chapterPurchase.count({
            where: { userId }
        }),
    ...
    const newlyUnlocked = [];
    // Check Helper
    const tryUnlock = async (achievementId, condition) => {
        if (condition && !unlockedIds.has(achievementId)) {
            const ach = await prisma_1.prisma.achievement.findUnique({ where: { id: achievementId } });
            if (ach) {
                // Unlock
                await prisma_1.prisma.$transaction([
                    prisma_1.prisma.userAchievement.create({
